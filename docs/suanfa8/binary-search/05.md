...
> **本节重点**：首先需要认识到一件事情：`int mid = (left + right) / 2` 和 `int mid = (left + right + 1) / 2` 的「地位」本应该是一致的，它们都是位于中间位置的元素。

在搜索区间里只有两个元素的时候，`int mid = (left + right) / 2` 取到的是中间位置靠左的元素的位置，`int mid = (left + right + 1) / 2` 取到的是中间位置靠右的元素的位置。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/suanfa8/202305261917196.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/suanfa8/202305261918185.png)
...
作者：liweiwei1419
链接：https://suanfa8.com/binary-search/endless-loop
来源：算法吧
