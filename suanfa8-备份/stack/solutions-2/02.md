...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/suanfa8/9013f22a-7720-44f2-8566-d426b25e2726.jpg)
...
> 阅读提示：本题刚刚接触的时候会觉得有一点复杂，所以我在「视频讲解」和「文字题解」的部分用了一些例子来讲解为什么 **可以使用「栈」（因为解决这个问题恰好符合了「后进先出」的顺序）**。内容有一些多，但是讲解是层层递进的，稍微有点耐心就可以看完。

## 🖥视频讲解

这道题在 [官方题解](https://leetcode-cn.com/problems/largest-rectangle-in-histogram/solution/zhu-zhuang-tu-zhong-zui-da-de-ju-xing-by-leetcode-/) 和 [B 站](https://www.bilibili.com/video/BV16D4y1D7ed?from=search&seid=10193465856501349147&spm_id_from=333.337.0.0) 可以收看视频讲解，可以点击下面的视频右上角「去 bilibili 观看」，选择快速播放，获得更好的观看体验。
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250106957.jpeg)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250107495.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250108542.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250109923.png)

2、然后看到到高度为 `1` 的柱形，这个时候以这个柱形为高度的矩形的最大面积还是不知道的。但是**它之前的以 `2` 为高度的最大面积的矩形是可以确定的**，这是因为这个 `1` 比 `2` 小 ，因为这个 `1` 卡在了这里 `2` 不能再向右边扩展了，如下图。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250109077.png)

我们计算一下以 `2` 为高度的最大矩形的面积是 `2`。其实这个时候，求解这个问题的思路其实已经慢慢打开了。如果已经确定了一个柱形的高度，我们可以无视它，将它以虚框表示，如下图。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250111515.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250111034.png)

4、接下来，遍历到高度为 `6` 的柱形，同样的，以柱形 `1`、`5`、`6` 为高度的最大矩形面积还是不能确定下来；

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250112639.png)

5、再接下来，遍历到高度为 `2` 的柱形。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250112689.png)

发现了一件很神奇的事情，**高度为 `6` 的柱形对应的最大矩形的面积的宽度可以确定下来**，它就是夹在高度为 `5` 的柱形和高度为 `2` 的柱形之间的距离，它的高度是 `6`，宽度是 `1`。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250112714.png)

将可以确定的柱形设置为虚线。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250109517.png)

接下来柱形 `5` 对应的最大面积的矩形的宽度也可以确定下来，它是夹在高度为 `1` 和高度为 `2` 的两个柱形之间的距离；

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250110755.png)

确定好以后，我们将它标成虚线。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250110926.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250113449.png)
...
![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250115351.png)

7、下标为 5 ，即高度为 3 的柱形，左边的下标是 4 ，右边的下标是 6 ，因此宽度是 6 - 4 - 1 = 1（两边都不算，只算中间的距离，所以减 1）；算完以后，将它标为虚线。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250116550.png)

8、下标为 4 ，高度为 2 的柱形，左边的下标是 1 ，右边的下标是 6 ，因此宽度是 6 - 1 - 1 = 4；算完以后，将它标为虚线。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250116248.png)

9、最后看下标为 1，高度为 1 的矩形，它的左边和右边其实都没有元素了，它就是整个柱形数组里高度最低的柱形，计算它的宽度，就是整个柱形数组的长度。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250116546.png)

到此为止，所有的柱形高度对应的最大矩形的面积就都计算出来了。

![](https://suanfa8-1252206550.cos.ap-shanghai.myqcloud.com/202301250116940.png)
...
作者：liweiwei1419
链接：https://suanfa8.com/stack/solutions-2/0084-largest-rectangle-in-histogram
来源：算法吧
